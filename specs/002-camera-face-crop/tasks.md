...
- [ ] T010 [P] Update `tests/regression/mosaic-pipeline.test.ts`: add two new tests using `makeRainbowGradient()` — one calling `generateMosaic(img, palette, { height: 28 })` with `toMatchFileSnapshot('__snapshots__/mosaic-32x28.snap.json')` and one with `{ height: 36 }` saving to `mosaic-32x36.snap.json`; verify existing 32×32 test still passes (depends on T005, T006)
...
- [ ] T020 [US2] Implement `src/lib/image/head-detection.ts`: export `initHeadDetector(): Promise<void>` — fetches `face_landmarker.task` from `/models/` and initializes `FaceLandmarker` from `@mediapipe/tasks-vision` with WASM backend; export `detectHeadBounds(imageData: ImageData): HeadBounds` — runs inference; computes `topY = landmark[10].y - 0.10 × faceHeight`, `bottomY = landmark[152].y`; returns `detectionStatus: 'too-small'` if detected height < 150px, `detectionStatus: 'not-found'` if no face (depends on T005)
...
- [ ] T022 [US2] Wire US2 preparing + head-crop flow in `src/pages/index.astro`: after `onFileValidated` or `onPhotoConfirmed` → transition to `preparing` → call `getOrientedImageData` → call `initHeadDetector` (lazy, once) then `detectHeadBounds` → call `onImagePrepared(headBounds)` → show `HeadCropTool` in `head-cropping` phase; handle brick height change → `onBrickHeightChanged`; handle `cropConfirmed` event from `HeadCropTool` → read `HeadCropSelection` from crop tool → `onHeadCropConfirmed` → transition to `adjusting`; handle reset → `onResetToFullImage`; apply `FR-017` fallback in `onImagePrepared`: if `headBounds.detectionStatus !== 'found'`, show non-blocking notice and proceed with full-image handles (depends on T008, T018, T019, T020, T021)
...
| 4 — US2 Head-Height Crop | P2 | T016–T022 (7) | T016, T017, T019 (3) |
| 5 — Polish | — | T023–T028 (6) | T023–T025 (3) |
| **Total** | | **28 tasks** | **14 parallelizable** |
